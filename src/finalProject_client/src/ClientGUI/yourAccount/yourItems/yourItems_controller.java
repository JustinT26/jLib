package ClientGUI.yourAccount.yourItems;
...
import java.net.URL;
import java.util.*;
public class yourItems_controller implements Initializable
{
    ...
    public void initialize(URL location, ResourceBundle resources)
    {
        ...
        if(loggedInUser.getCurrentCheckedOutItems().isEmpty()) // no items to show
            scrollPane.setVisible(false);
        else
        {
            //set the VBox children anchorpanes with corresponding info
            for(int i = 0; i < loggedInUser.getCurrentCheckedOutItems().size(); i++)
            {
                AnchorPane newResultPane = ((AnchorPane)userItemsVBox.getChildren().get(i));
                newResultPane.setVisible(true);

                //set the info
                Item item = Client.getUpdatedItem_fromServer(loggedInUser.getCurrentCheckedOutItems().get(i));
            ...
            userItemsVBox.getChildren().remove(loggedInUser.getCurrentCheckedOutItems().size(), userItemsVBox.getChildren().size());
        }
